...
import {clamp,nativeEventProps} from './helpers';
...
export const useWidthLayout = (
  ...
  return useCallback(
     //TODO
    //here i add type for nativeEvent
    ({nativeEvent}: { nativeEvent: nativeEventProps }) => {
      const {
        layout: {width},
      } = nativeEvent;
...
export const useThumbFollower = (
  ...
  const update = useCallback(
    ...
    //TODO
    //here i add types for thumbPositionInView && value
...
export const useLabelContainerProps = (floating: boolean) => {
  const [labelContainerHeight, setLabelContainerHeight] = useState(0);
    ...
    //TODO
    //here i add types for nativeEvent
  const onLayout = useCallback(({nativeEvent}:{ nativeEvent: nativeEventProps }) => {
    const {
      layout: {height},
    } = nativeEvent;
